...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.location.edit') }}</h1>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    {{ Form::model($location, ['id' => 'editForm', 'route' => ['locations.update', $location->id], 'method' => 'put', 'files' => true]) }}
                    <div class="row">
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            {{ Form::select('parent', $parentOptions, $location->parent, ['class' => 'form-control', 'placeholder' => 'Select Parent Location']) }}
                        </div>
                        <div class="form-group col-sm-6">
                            {{ Form::label('manager', __('messages.location.manager').':') }}
                            {{ Form::select('manager', $managerOptions, $location->manager, ['class' => 'form-control', 'placeholder' => 'Select Manager']) }}
                        </div>
                        <div class="form-group col-sm-6">
                            {{ Form::label('location_currency', __('messages.location.location_currency').':') }}
                            {{ Form::select('location_currency', $currencyOptions, $location->location_currency, ['class' => 'form-control', 'placeholder' => 'Select Currency']) }}
                        </div>
                        ...
                        <div class="form-group col-sm-6">
                            ...
                            @if($location->image)
                                <div class="mt-2">
                                    <img src="{{ asset('storage/'.$location->image) }}" width="100" class="img-thumbnail">
                                </div>
                            @endif
                        </div>
                    </div>
