...
@section('content')
    ...
    <section class="section">
            ...
            <div class="float-right d-flex">
                <div class="dropdown export-dropdown mr-2">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="exportDropdown">
                        ...
                        <a class="dropdown-item" href="{{ route('booking_lists.export', ['format' => 'xlsx']) }}">
                            <i class="fas fa-file-excel text-primary mr-2"></i> {{ __('messages.common.excel') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('booking_lists.export', ['format' => 'print']) }}"
                            target="_blank">
                ...
                <button type="button" class="btn btn-success btn-sm form-btn mr-2" id="bookingListImportButton">
        ...
        <!-- BookingList Import Modal -->
        <div class="modal fade" id="bookingListImportModal" tabindex="-1" role="dialog"
            aria-labelledby="bookingListImportModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <form action="{{ route('booking_lists.import') }}" method="POST" enctype="multipart/form-data"
                    id="bookingListImportForm">
                    @csrf
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="bookingListImportModalLabel">
                                {{ __('Import Booking Lists via CSV') }}</h5>
                        ...
                        <div class="modal-body">
                            <a href="{{ route('booking_lists.sample-csv') }}" class="btn btn-info btn-sm mb-3">
                                <i class="fas fa-download mr-1"></i> {{ __('Download Sample CSV') }}
                            </a>

                            <div class="form-group">
                                <label for="bookingListCsvFile">{{ __('Upload CSV File') }}</label>
                                <input type="file" name="file" class="form-control-file" id="bookingListCsvFile"
                                    required>
                            </div>
                        </div>
...
@section('scripts')
        ...
        let bookingListTable = $('#bookingListTable').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    data: 'booking_number',
                    name: 'booking_number',
                    title: "{{ __('messages.booking_lists.booking_number') }}",
                    width: '15%'
                },
                {
                    data: 'room_type',
                    name: 'room_type',
                    title: "{{ __('messages.booking_lists.room_type') }}",
                    width: '15%'
                },
                {
                    data: 'room_no',
                    name: 'room_no',
                    title: "{{ __('messages.booking_lists.room_no') }}",
                    width: '10%'
                },
                {
                    data: 'check_in',
                    name: 'check_in',
                    title: "{{ __('messages.booking_lists.check_in') }}",
                    width: '15%'
                },
                {
                    data: 'check_out',
                    name: 'check_out',
                    title: "{{ __('messages.booking_lists.check_out') }}",
                    width: '15%'
                },
                {
                    data: 'booking_status',
                    ...
                    render: function(data) {
                        return data ? "{{ __('messages.booking_lists.success') }}" :
                            "{{ __('messages.booking_lists.pending') }}";
                    }
                },
                {
                    data: function(row) {
    ...
    <script>
        $(document).ready(function() {
            $('#bookingListImportModal').modal('hide');
            $('.modal').removeClass('show');
            $('body').removeClass('modal-open');
            $('.modal-backdrop').remove();

            $('#bookingListImportModal').css({
                'display': 'none',
                'padding-right': '0px'
            });

            $('#bookingListImportButton').on('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                $('#bookingListImportModal').modal('show');
                window.manuallyOpenedBookingList = true;
            });

            $('#bookingListImportModal').on('shown.bs.modal', function() {
                $('#bookingListCsvFile').focus();
            });

            $('#bookingListImportModal').on('hidden.bs.modal', function() {
                $('#bookingListImportForm')[0].reset();
                window.manuallyOpenedBookingList = false;
            });

            setTimeout(function() {
                if ($('#bookingListImportModal').hasClass('show') && !window.manuallyOpenedBookingList) {
                    $('#bookingListImportModal').modal('hide');
                    $('body').removeClass('modal-open');
                    $('.modal-backdrop').remove();
                }
