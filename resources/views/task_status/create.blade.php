                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-md-6">
                            {{ Form::label('duration', __('messages.task-status.duration')) }}<span
                                class="required">*</span>
                            {{ Form::number('duration', null, ['class' => 'form-control', 'required', 'id' => 'task_duration', 'step' => '0.01']) }}
                        </div>
                        <div class="form-group col-sm-12">
                            {{ Form::label('task', __('messages.task-status.task')) }}<span class="required">*</span>
                            {{ Form::textarea('task', null, ['class' => 'form-control', 'required', 'id' => 'task_name', 'style' => 'height: 80px;']) }}

                        </div>
                        <div class="form-group col-sm-12 mb-0">
                            {{ Form::label('description', __('messages.task-status.description')) }}
                            {{ Form::textarea('description', null, ['class' => 'form-control summernote-simple', 'id' => 'task_description']) }}
                        </div>

                        <div class="form-group col-md-6 col-sm-12">
                            {{ Form::label('customer_id', __('messages.task-status.customer')) }}<span
                                class="required">*</span>
                            {{ Form::select('customer_id', $customers, null, ['class' => 'form-control select2', 'required', 'id' => 'customer_id', 'placeholder' => __('messages.placeholder.select_customer')]) }}
                        </div>
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            {{ Form::label('project_id', __('messages.task-status.project')) }}<span
                                class="required">*</span>
                            {{ Form::select('project_id', [], null, ['class' => 'form-control select2', 'required', 'id' => 'project_id', 'placeholder' => __('messages.placeholder.select_project')]) }}
                        </div>
...
@section('scripts')
    ...
    <script>
        let departmentNewCreateUrl = route('task-status.store');
        $(document).on('submit', '#addNewFormDepartmentNew', function(event) {
            ...
            $.ajax({
                url: departmentNewCreateUrl, // Ensure the correct URL is set
                type: 'POST',
