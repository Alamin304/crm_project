            ...
            <div class="float-right d-flex">
                <div class="dropdown export-dropdown mr-2">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="exportDropdown">
                        <a class="dropdown-item" href="{{ route('buy_requests.export', ['format' => 'pdf']) }}">
                            {{ __('PDF') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('buy_requests.export', ['format' => 'csv']) }}">
                            {{ __('CSV') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('buy_requests.export', ['format' => 'xlsx']) }}">
                            {{ __('Excel') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('buy_requests.export', ['format' => 'print']) }}"
                            target="_blank">
                            {{ __('Print') }}
                        </a>
                    </div>
                ...
                <div class="float-right">
                    <a href="{{ route('buy_requests.create') }}" class="btn btn-primary form-btn">
                        {{ __('Add') }}
                    </a>
                </div>
            </div>
...
@section('scripts')
    <script>
        ...
        let buyRequestTable = $('#buyRequestTable').DataTable({
            oLanguage: {
        ...
        $(document).on('click', '.delete-btn', function(event) {
            ...
            deleteItem("{{ route('buy_requests.destroy', ['buyRequest' => ':id']) }}".replace(':id', id),
                '#buyRequestTable', "{{ __('Buy Request') }}");
        });
        ...
        $(document).on('change', '.status-select', function() {
            ...
            $.ajax({
                url: "{{ route('buy_requests.update-status', ['buyRequest' => ':id']) }}".replace(
                    ':id', id),
                type: 'PUT',
                data: {
                    status: status,
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        $('#buyRequestTable').DataTable().ajax.reload(null, false);
                    }
                },
        ...
        function renderActionButtons(id) {
            let deleteUrl = "{{ route('buy_requests.destroy', ':id') }}".replace(':id', id);
            let viewUrl = "{{ route('buy_requests.view', ':id') }}".replace(':id', id);
            let editUrl = "{{ route('buy_requests.edit', ':id') }}".replace(':id', id);
