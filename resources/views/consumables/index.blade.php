...
@section('content')
    ...
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.consumables.consumables') }}</h1>
            ...
            <div class="float-right d-flex">
                <div class="dropdown export-dropdown mr-2">
                    ...
                    <div class="dropdown-menu dropdown-menu-right" aria-labelledby="exportDropdown">
                        <a class="dropdown-item" href="{{ route('consumables.export', ['format' => 'pdf']) }}">
                            {{ __('messages.common.pdf') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('consumables.export', ['format' => 'csv']) }}">
                            {{ __('messages.common.csv') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('consumables.export', ['format' => 'xlsx']) }}">
                            {{ __('messages.common.excel') }}
                        </a>
                        <a class="dropdown-item" href="{{ route('consumables.export', ['format' => 'print']) }}"
                            target="_blank">
                ...
                <a href="{{ route('consumables.create') }}" class="btn btn-primary form-btn">
                    {{ __('messages.consumables.add') }}
                </a>
            </div>
        </div>
        ...
        <div class="modal fade" id="accessoryImportModal" tabindex="-1" role="dialog"
            ...
            <div class="modal-dialog" role="document">
                <form action="{{ route('consumables.import') }}" method="POST" enctype="multipart/form-data"
                    id="accessoryImportForm">
                    @csrf
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="accessoryImportModalLabel">{{ __('Import Consumables via CSV') }}
                            </h5>
                        ...
                        <div class="modal-body">
                            <a href="{{ route('consumables.sample-csv') }}" class="btn btn-info btn-sm mb-3">
            ...
            <div class="card">
                <div class="card-body">
                    <table
                        ...
                        <thead>
                            <tr>
                                <th>{{ __('messages.consumables.image') }}</th>
...
@section('scripts')
    <script>
        ...
        let consumablesTable = $('#consumablesTable').DataTable({
            oLanguage: {
        ...
        $(document).on('click', '.delete-btn', function(event) {
            ...
            deleteItem("{{ route('consumables.destroy', ':id') }}".replace(':id', accessoryId), '#accessoryTable',
                "{{ __('messages.consumables.consumables') }}");
        });

        function renderActionButtons(id) {
            let deleteUrl = "{{ route('consumables.destroy', ':id') }}".replace(':id', id);
            let viewUrl = "{{ route('consumables.show', ':id') }}".replace(':id', id);
            let editUrl = "{{ route('consumables.edit', ':id') }}".replace(':id', id);
