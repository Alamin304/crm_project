@extends('layouts.app')
@section('title')
    {{ __('messages.divisions.divisions') }}
@endsection
...
@section('content')
 ...
 {{-- Success Message --}}
    ...
    <section class="section">
        ...
        <!-- Division Import Modal -->
        <div class="modal fade" id="divisionImportModal" tabindex="-1" role="dialog"
            aria-labelledby="divisionImportModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <form action="{{ route('divisions.import') }}" method="POST" enctype="multipart/form-data"
                    id="divisionImportForm">
                    @csrf
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="divisionImportModalLabel">{{ __('Import Divisions via CSV') }}</h5>
                        ...
                        <div class="modal-body">
                            <a href="{{ route('divisions.sample-csv') }}" class="btn btn-info btn-sm mb-3">
                                <i class="fas fa-download mr-1"></i> {{ __('Download Sample CSV') }}
                            </a>

                            <div class="form-group">
                                <label for="divisionCsvFile">{{ __('Upload CSV File') }}</label>
                                <input type="file" name="file" class="form-control-file" id="divisionCsvFile"
                                    required>
                            </div>
                        </div>
...
@section('scripts')
    <script>
        ...
        let divisionTable = $('#divisionTable').DataTable({
            oLanguage: {
        ...
        $(document).on('click', '.delete-btn', function(event) {
            let divisionId = $(event.currentTarget).data('id');
            deleteItem("{{ route('divisions.destroy', ':id') }}".replace(':id', divisionId), '#divisionTable',
                "{{ __('messages.divisions.divisions') }}");
        });

        function renderActionButtons(id) {
            let deleteUrl = "{{ route('divisions.destroy', ':id') }}".replace(':id', id);
            let viewUrl = "{{ route('divisions.view', ':id') }}".replace(':id', id);
            let editUrl = "{{ route('divisions.edit', ':id') }}".replace(':id', id);
    ...
    <script>
        $(document).ready(function() {
            $('#divisionImportModal').modal('hide');
            $('.modal').removeClass('show');
            $('body').removeClass('modal-open');
            $('.modal-backdrop').remove();

            $('#divisionImportModal').css({
                'display': 'none',
                'padding-right': '0px'
            });

            $('#divisionImportButton').on('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
                $('#divisionImportModal').modal('show');
                window.manuallyOpenedDivision = true;
            });

            $('#divisionImportModal').on('shown.bs.modal', function() {
                $('#divisionCsvFile').focus();
            });

            $('#divisionImportModal').on('hidden.bs.modal', function() {
                $('#divisionImportForm')[0].reset();
                window.manuallyOpenedDivision = false;
            });

            setTimeout(function() {
                if ($('#divisionImportModal').hasClass('show') && !window.manuallyOpenedDivision) {
                    $('#divisionImportModal').modal('hide');
                    $('body').removeClass('modal-open');
                    $('.modal-backdrop').remove();
                }
